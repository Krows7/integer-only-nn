...
#define LEARNING_RATE_MU 4  // Corresponds to gradient bit shift (adjust as needed)
...
// (Identical to the one in digits-test-run.c - Reusable)
Vector8* predict_mnist(Network* network, Matrix8* x_batch) {
...
void evaluate_mnist(Network* network, float** X_float_test, Vector8* Y_test, int num_test_samples, float** X_float_train, Vector8* Y_train, int num_train_samples) {
    ...
    for (int sample_idx = 0; sample_idx < num_test_samples; sample_idx += BATCH_SIZE) {
        ...
        Vector8* predicted = predict_mnist(network, &x_batch);
    ...
    for (int sample_idx = 0; sample_idx < num_train_samples; sample_idx += BATCH_SIZE) {
        int current_batch_size = (sample_idx + BATCH_SIZE <= num_train_samples) ? BATCH_SIZE : (num_train_samples - sample_idx);
        if (current_batch_size <= 0) continue;

        Matrix8 x_batch = create_quantized_batch(X_float_train, sample_idx, current_batch_size, INPUT_SIZE);
        Vector8 y_batch = v_cpy_range(Y_train, sample_idx, sample_idx + current_batch_size); // end index is exclusive
    ...
    accuracy = (samples_processed > 0) ? (double)correct_predictions / samples_processed * 100.0 : 0.0;

    printf("Accuracy on MNIST train set: %.2f%% (%d / %d correct)\n", accuracy, correct_predictions, samples_processed);
}
...
int main(void) {
    ...
    network->layers[0] = init_layer(BATCH_SIZE, INPUT_SIZE, HIDDEN_NEURONS, LINEAR);
    ...
    network->layers[1] = init_layer(BATCH_SIZE, HIDDEN_NEURONS, HIDDEN_NEURONS, RELU);
    ...
    network->layers[2] = init_layer(BATCH_SIZE, HIDDEN_NEURONS, NUM_CLASSES, LINEAR);
    ...
    for (uint16_t epoch = 0; epoch < NUM_EPOCHS; ++epoch) {
        ...
        if (num_test_samples_loaded > 0) {
            evaluate_mnist(network, X_test_float, &Y_test_full, num_test_samples_loaded, X_train_float, &Y_train_full, num_train_samples_loaded);
        } else {
