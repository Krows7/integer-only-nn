...
uint8_t urand8(void)   { return lfsr8_step(); }

/// signed 8-bit  (casts 0..255 → −128..127)
__attribute__((leaf)) char rand8(void)    { return (int8_t)lfsr8_step(); }

/// unsigned 16-bit [1..65535]
uint16_t urand16(void) { return lfsr16_step(); }

/// signed 16-bit (casts 0..65535 → −32768..32767)
__attribute__((leaf)) unsigned rand16(void)  { return (int16_t)lfsr16_step(); }

/*——————————————————————————————————————————————————————————————————————
  — Optional: allow the user to seed; must not set state to 0
  ——————————————————————————————————————————————————————————————————————*/
inline void srand8(uint8_t seed) {
    if (seed == 0) seed = 1;
    lfsr8_state = seed;
}

inline void srand16(uint16_t seed) {
    if (seed == 0) seed = 1;
    lfsr16_state = seed;
}