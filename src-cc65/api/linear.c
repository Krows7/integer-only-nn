...
void lin_cleanup() {
    ...
    for (i = 0; i < m_pool->size; i++) {
        for (j = 0; j < m_pool->matrices[i]->width; ++j) {
    ...
    for (i = 0; i < m8_pool->size; i++) {
        for (j = 0; j < m8_pool->matrices[i]->width; ++j) {
    ...
    if (m8_pool->size < m8_pool->capacity) {
        ...
        *m8_pool->matrices[m8_pool->size] = init_m8_(width, height);
        ...
        if (first_free != m8_pool->capacity) {
            ...
            for (j = 0; j < m8_pool->matrices[first_free]->width; ++j) {
    ...
    if (m_pool->size < m_pool->capacity) {
        ...
        *m_pool->matrices[m_pool->size] = init_m32_(width, height);
        ...
        if (first_free != m_pool->capacity) {
            ...
            for (j = 0; j < m_pool->matrices[first_free]->width; ++j) {
...
void mul8_1t(const Matrix8* A_T, const Matrix8* B, Matrix32* C) {
    ...
    for (i = 0; i < A_T->height; ++i) {
        for (j = 0; j < B->height; ++j) {
            C->matrix[i][j] = 0;
            for (k = 0; k < A_T->width; k++) {
